...
@protocol COToolbarProtocol <NSObject>
...
@interface COToolbar : NSToolbar <NSToolbarDelegate, NSToolbarItemValidation>

// bar use mode, as in the mode which affects how the bar would be used

typedef NS_ENUM(NSUInteger, COToolbarMode)
{
	kCOToolbarModeDud,
	...
	kCOToolbarModeNotInstalledPackage,
	kCOToolbarModeInstalledPackage,
    
	kCOToolbarModeOutdatedPackage,
    
    /// added taps page, nothing selected
	kCOToolbarModeTap,
	kCOToolbarModeTappedRepository
};

@property (nonatomic) COToolbarMode mode;
