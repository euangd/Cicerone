...
@interface COToolbar () <NSSearchFieldDelegate, NSToolbarItemValidation>
...
- (instancetype)initWithIdentifier:(NSString *)identifier
{
	...
	if (self)
	{
        self.sizeMode = [COStyle toolbarSize];
        self.showsBaselineSeparator = NO;

        self.delegate = self;
        
        _mode = (COToolbarMode)-1;
        self.mode = kCOToolbarModeDud;
	}
    
	return self;
}

// NSToolbar, NSToolbarItemValidation; called from COHomebrewViewController <NSToolbarItemValidation> via Cocoa because item.target = _homebrewViewController and sometimes manually

- (BOOL)validateToolbarItem:(NSToolbarItem *)item
{
    switch (self.mode) {
        case kCOToolbarModeDud:
            return NO;
    }
...
- (void)setMode:(COToolbarMode)mode
{
	@synchronized (self) {
        if (self.mode == mode)
        {
...
- (void)actualizeToolbarItem:(NSToolbarItem *)item
{
    if ([item.itemIdentifier isEqualToString:kToolbarItemInstallLatestPackageVersionIdentifier]) {
        switch (self.mode) {
            case kCOToolbarModeInstalledPackage:
            case kCOToolbarModeNotInstalledPackage:
                [self modifyToolbarItem:item
                             withVisual:[COStyle toolbarImageForInstall]
            ...
            case kCOToolbarModeOutdatedPackage:
                [self modifyToolbarItem:item
                             withVisual:[COStyle toolbarImageForUpgrade]
        ...
        switch (self.mode) {
            ...
            case kCOToolbarModeInstalledPackage:
                [self modifyToolbarItem:item
                             withVisual:[COStyle toolbarImageForUninstall]
            ...
            case kCOToolbarModeTappedRepository:
                [self modifyToolbarItem:item
                             withVisual:[COStyle toolbarImageForUntap]
...
- (NSToolbarItem *)brewTapToolToolbarItem
{
    if (!_brewTapToolToolbarItem) {
        _brewTapToolToolbarItem = [self toolbarItemWithIdentifier:kToolbarItemBrewTapToolIdentifier
                                                       withVisual:[COStyle toolbarImageForTap]
...
- (NSToolbarItem *)brewUpdateToolToolbarItem
{
	if (!_brewUpdateToolToolbarItem)
	{
        _brewUpdateToolToolbarItem = [self toolbarItemWithIdentifier:kToolbarItemBrewUpdateToolIdentifier
                                                          withVisual:[COStyle toolbarImageForUpdate]
...
- (NSToolbarItem *)brewInfoToolToolbarItem
{
	if (!_brewInfoToolToolbarItem)
	{
		_brewInfoToolToolbarItem = [self toolbarItemWithIdentifier:kToolbarItemBrewInfoToolIdentifier
                                                        withVisual:[COStyle toolbarImageForMoreInformation]
