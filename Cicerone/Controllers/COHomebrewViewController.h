...
typedef NS_ENUM(NSUInteger, COWindowOperation) {
	kCOWindowOperationInstall,
	kCOWindowOperationUninstall,
	kCOWindowOperationUpgrade,
	kCOWindowOperationTap,
	kCOWindowOperationUntap,
	kCOWindowOperationCleanup
};

@class COUpdateDoctorController;

@interface COHomebrewViewController : NSViewController

@property (weak) IBOutlet COSideBarController      *sidebarController;
@property (weak) IBOutlet COFormulaeTableView      *formulaeTableView;
...
@property (nonatomic, readonly) COFormula *selectedFormula; // used to be (copy) but then -[setSelectedFormula:] would freeze, at least when @synchronized (self) up the call stack
